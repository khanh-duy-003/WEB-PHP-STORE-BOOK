...
$products = Product::where('status', '!=', '0')
   ...
   foreach ($categories as $cat) {
      if ($cat->parent_id == 0) {
         $str_category_id .= "<option value='" . $cat->id . "'>" . $cat->name . "</option>";
         foreach ($categories as $cat1) {
            if ($cat1->parent_id == $cat->id) {
               $str_category_id .= "<option value='" . $cat1->id . "'>--- " . $cat1->name . "</option>";
               foreach ($categories as $cat2) {
                  if ($cat2->parent_id == $cat1->id) {
                     $str_category_id .= "<option value='" . $cat2->id . "'>------ " . $cat2->name . "</option>";
                  }
               }
            }
         }
   ...
   {
     $str_brand_id .="<option value='" .$item->id."'>".$item->name."</option>";
   }
?>
   ...
   <form action="index.php?opt=product&cat=process" method="post" enctype="multipart/form-data">
      ...
      <section class="content-header my-2">
         ...
         <div class="mt-1 text-end">
            <a class="btn btn-sm btn-primary" href="?opt=product">
